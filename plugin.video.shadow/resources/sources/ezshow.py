...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if 1:
        ...
        x=requests.get('http://eztv.show/?s='+search_url.replace(' ','%20'),headers=base_header,timeout=10).content
        logging.warning('http://eztv.show/?s='+search_url.replace(' ','%20'))
        ...
        for link,title in macth_pre:
                ...
                if clean_name(original_title,1).lower() in title.lower() and 's%se%s'%(season_n,episode_n) in title.lower() :
                     y=requests.get(link,headers=base_header,timeout=10).content
        
                     regex='strong>Download Torrent: </strong> <a href="(.+?)"'
                     lk=re.compile(regex).findall(y)
                     if len(lk)>0:
                         lk=lk[0]
                         logging.warning('In')
                         if stop_all==1:
