...
class HLSMLB():
    ...
    def init(self, out_stream, url, proxy=None, use_proxy_for_chunks=True, g_stopEvent=None, maxbitrate=0, auth='', callbackpath="", callbackparam="", replace_to=None, replace_from=None):
        global clientHeader, gproxy, gauth, c, replace_from_g, replace_to_g
        try:
            self.init_done = False
            self.init_url = url
            clientHeader = None
            ...
            if replace_to == None or replace_to == 'None' or replace_to == '':
                replace_to = None
            if replace_to:
                replace_to_g = replace_to
                
            if replace_from == None or replace_from == 'None' or replace_from == '':
                replace_from = None
            if replace_from:
                replace_from_g = replace_from
            ...
            self.url = url
            # disabled
            # downloadInternal(self.url,None,self.maxbitrate,self.g_stopEvent ,
            # self.callbackpath,  self.callbackparam, testing=True)
            return True
        except Exception as e:
            LOG('Error en HLSMLB init:', e)
            #===================================================================
            # traceback.print_exc()
            #===================================================================
            ...
            if tag == '#EXTINF':
                ...
                if attribs['METHOD'] == 'NONE':
                    ...
                    if not aesdone:
                        ...
                        if replace_to_g and replace_from_g and replace_from_g in codeurl:
                            LOG ('replace')
                            codeurl = six.ensure_binary(codeurl)
                            replace_to_g = six.ensure_str(replace_to_g)
                            codeurl = replace_to_g + six.ensure_str(base64.b64encode(codeurl))
    ...
    try:
        ...
        while 1 == 1:  # thread.isAlive():
            ...
            for media in medialist:
                ...
                if len(medialist) > 20 and last_seq == -1:
                    last_seq = len(medialist) - 21
                if seq > last_seq:
