...
from resources.lib.F4mProxy import akhds
...
def urlencode_param(value):
    ...
    return urllib_parse.quote_plus(value, safe=VALUE_SAFE)
...
class FlvReader(io.BytesIO):
    ...
    def read_abst(self, debug=False):
        ...
        streamType = self.read_unsigned_char()
        islive = False
...
def build_fragments_list(boot_info, startFromFregment=None, live=True):
    ...
    # segment_run_table['segment_run']=segment_run_table['segment_run'][-2:]
    # #pick latest
...
#    sum(j for i, j in segment_run_table['segment_run'])
    ...
    for current in range(len(segment_run_table['segment_run'])):
        ...
        segment_run_table['segment_run'][current] = (
            seg, fregCount, first_frag_number, frag_end)
...
#               startFromFregment= boot_info['fragments'][0]['fragments'][-1]['first']
            ...
            # start from begining
            startFromFregment = boot_info['fragments'][0]['fragments'][0]['first']
...
def join(base, url):
    join = urllib_parse.urljoin(base, url)
    url = urllib_parse.urlparse(join)
    path = posixpath.normpath(url[2])
    return urllib_parse.urlunparse((url.scheme, url.netloc, path, url.params, url.query, url.fragment))
...
class F4MDownloader():
    ...
    def getUrl(self, url, ischunkDownloading=False, timeout=15, stream=False):
        global cookieJar
        global clientHeader
        global nsplayer
    ...
    def getUrl_old(self, url, ischunkDownloading=False):
        try:
            post = None
            print ('url', url)

            cookie_handler = urllib_request.HTTPCookieProcessor(self.cookieJar)
        ...
        try:
            ...
            if '\"bootstrapInfoId' in manifest:
                manifest = manifest.replace(
                    '\"bootstrapInfoId', '\" bootstrapInfoId')
            ...
            self.metadata = None
            try:
                self.metadata = base64.b64decode(
                    media.find(_add_ns('metadata')).text)
                # ,media.find(_add_ns('metadata')).text
            ...
            # compat_urllib_parse.urljoin(man_url,media.attrib['url'])
            base_url = join(man_url, mediaUrl)
            ...
            if bootstrapURL1 == '':
                bootstrapData = base64.b64decode(
                    doc.findall(_add_ns('bootstrapInfo'))[0].text)
                #
            else:
                queryString = urllib_parse.urlparse(url).query
                ...
                if queryString == None or '?' in bootstrap.attrib['url']:
                    ...
                    # take out querystring for later
                    bootstrapURL = join(man_url, bootstrap.attrib['url'])
                    queryString = urllib_parse.urlparse(bootstrapURL).query
                    print ('queryString override', queryString)
                    if len(queryString) == 0:
                        queryString = None
                        if len(self.auth) > 0:
                            bootstrapURL += '?' + self.auth
                            # self._pv_params('',self.auth20)#not in use
                            queryString = self.auth
                        elif len(self.auth20) > 0:

                            queryString = self._pv_params(
                                self.swf, self.auth20)
                            bootstrapURL += '?' + queryString
                else:
                    print ('queryString!!', queryString)
                    bootstrapURL = join(
                        man_url, bootstrap.attrib['url']) + '?' + queryString
                    if len(self.auth) > 0:
                        # self._pv_params('',self.auth20)#not in use
                        authval = self.auth
                        bootstrapURL = join(
                            man_url, bootstrap.attrib['url']) + '?' + authval
                        queryString = authval
                    elif len(self.auth20) > 0:
                        authval = self._pv_params(
                            self.swf, self.auth20)  # requires swf param
                        bootstrapURL = join(
                            man_url, bootstrap.attrib['url']) + '?' + authval
                        queryString = authval
            ...
            self.bootstrap, self.boot_info, self.fragments_list, self.total_frags = self.readBootStrapInfo(
                bootstrapURL, bootstrapData)
            self.init_done = True
            return True
        except:
            traceback.print_exc()
...
#               print 'IV exists'
    ...
    def getFrames(self, box_data, remainingdata):
        ...
        while(KeepProcessing and lookForTagStart < len(box_data)):
            ...
            tagLen = ord(box_data[lookForTagStart + 1]) << 16 | ord(
                box_data[lookForTagStart + 2]) << 8 | ord(box_data[lookForTagStart + 3]) & 255
            ...
            if (nextTag > len(box_data) and currentStep > 0):
                ...
                chunk = []
                chunk = box_data[lookForTagStart:lookForTagStart + 
                                 tagLen + 11 + 4]
    ...
    def keep_sending_video(self, dest_stream, segmentToStart=None, totalSegmentToSend=0):
        try:
            ...
            self.downloadInternal(self.url, dest_stream,
                                  segmentToStart, totalSegmentToSend)
        except:
            traceback.print_exc()
        try:
            akhds.cleanup()
...
#                            print 'first byte',repr(box_data[0]),'kk'
                            ...
                            isAkamaiEncrypted = True if ord(box_data[0]) == 11 or ord(
                                box_data[0]) == 10 else False
                            if isAkamaiEncrypted:
                                # print 'Total MDAT count',len(box_data),
                                # len(box_data)%16

                                _loc8_ = ord(box_data[1]) << 16 | ord(
                                    box_data[2]) << 8 | ord(box_data[3]) & 255
                                _loc9_ = box_data[11:11 + _loc8_]
# print 'this is encrypted',len(_loc9_),_loc8_,repr(box_data[1:70])
                                keyExists, Key, dataread, lastIV = self.readAKKey(
                                    _loc9_)
                                if keyExists:
                                    # print 'key exists and its len is
                                    # ',_loc8_,repr(Key)
                                    doDecrypt = True
...
# print 'before frames data', repr(box_data[0:70])
                                    frames, remainingFrameData = self.getFrames(
                                        box_data, remainingFrameData)
#                                    print 'after frames data first frame', repr(frames[0][0:70])
                                    ...
                                    for frame in frames:
                                        ...
                                        data = frame[1]
                                        datalen = ord(data[1]) << 16 | ord(
                                            data[2]) << 8 | ord(data[3]) & 255
                                        preFrame = len(data)
                                        # print
                                        # 'samp>',len(data),datalen,ord(data[0])
                                        # ,'<samp'
                                        if firstPacket:
                                            firstPacket = False
#                                            data=data[0:datalen]
...
#                                        else:
                                        if 1 == 1:
                                            # if not not key frame then decrypt
                                            # else
                                            firstByte = ord(data[0])
                                            frameHeader = data[:11]
                                            framePad = data[11 + 
                                                            datalen:11 + datalen + 4]
                                            ...
                                            if firstByte == 10 or firstByte == 11:
                                                ...
                                                # print
                                                # 'sub>',len(data),firstByte,datalen,datalen%16,len(data)%16
                                                # ,'<sub'
                                                keyExistsNew, KeyNew, dataread, ignoreIV = self.readAKKey(
                                                    data)
# print 'dataread',dataread,keyExistsNew,KeyNew,ignoreIV

                                                try:
                                                    akhds.init()
                                                    data = akhds.tagDecrypt(
                                                        data, keyData)
...
# print 'pre return size %d, %d %d'%(len(frameHeader),len(data),
# len(framePad))
                                                seglen1 = self.addBytesToOutput(
                                                    frameHeader, data, framePad, 1, dest_stream)
                                                seglen += seglen1
...
# len(framePad))
                                                seglen1 = self.addBytesToOutput(
                                                    frameHeader, None, None, 1, dest_stream)
                                                seglen += seglen1
#                                                print 'pre frame %d, after %d'%(preFrame,seglen1)
                ...
                if self.seqNumber == len(fragments_list) or (totalSegmentToSend > 0 and frameSent == totalSegmentToSend):
                    if not self.live:
                        break
                    self.seqNumber = 0
                    # todo if the url not available then get manifest and get
                    # the data again
                    total_frags = None
                    try:
                        bootstrap, boot_info, fragments_list, total_frags = self.readBootStrapInfo(
                            self.bootstrapURL, None, updateMode=True, lastSegment=seg_i, lastFragement=frag_i)
                    except:
                        traceback.print_exc()
                        pass
                    if total_frags == None:
    ...
    def readBootStrapInfo(self, bootstrapUrl, bootStrapData, updateMode=False, lastFragement=None, lastSegment=None):
        ...
        try:
            ...
            while retries <= 10:
                ...
                # base64.b64decode(bootStrapData)#doc.findall(_add_ns('bootstrapInfo'))[0].text)
                ...
                fragments_list = build_fragments_list(
                    boot_info, newFragement, self.live)
...
#        pv="ZXhwPTE0Mzk2MDgzMTl+YWNsPSUyZip+ZGF0YT1wdmMsc35obWFjPTExYTJiNzQ4NjQyYmY1M2VlNzk5MzhhNTMzNjc1MTAzZjk2NWViOGVhODY4MzUwODkwZGM1MjVmNjI3ODM4MzQ="
        ...
        if data == "":
            return hdntl
        first_stage_msg = binascii.unhexlify(
            '056377146640142763057567157640125041016376130175171220177717044510157134116364123221072012122137150351003442036164015632157517073355151142067436113220106435137171174171127530157325044270025004')
...
#            print '********* NOOOOOOOOOOOOOOOOOOOOTTTTTTTTTTTTTTTTT**********'
        ...
        second_stage_key = hmac.new(
            first_stage_key, first_stage_msg, hashlib.sha256).digest()
...
#        print len(second_stage_data),repr(second_stage_data)
        ...
        third_stage_key = hmac.new(
            second_stage_key, second_stage_data, hashlib.sha256).digest()

        # hash=shouldhash
        msg = "exp=9999999999~acl=%2f%2a~data={0}!{1}".format(data, hash)

        auth = hmac.new(third_stage_key, msg.encode("ascii"), hashlib.sha256)
        ...
        params = urllib_parse.urlencode({'pvtoken': pvtoken}) + '&' + hdntl + '&hdcore=2.11.3'
