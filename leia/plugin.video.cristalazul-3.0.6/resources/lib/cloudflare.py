...
def createCookie(url, cj=None, agent='Mozilla/5.0 (Windows NT 6.1; rv:14.0) Gecko/20100101 Firefox/14.0.1'):
    ...
    try:
        ...
        opener = urllib_request.build_opener(NoRedirection, urllib_request.HTTPCookieProcessor(cj))
...
#        print response.headers
        ...
        answer = float('%.10f' % decryptVal) + len(urllib_parse.urlparse(url).netloc)

        u = '/'.join(url.split('/')[:-1])
        if '<form id="challenge-form" action="/cdn' in urlData:
            u = '/'.join(url.split('/')[:3])
        query = urllib_parse.urljoin(url, '/cdn-cgi/l/chk_jschl?s=%s&jschl_vc=%s&jschl_answer=%s' % (urllib_parse.quote_plus(sval), jschl, answer))
        passval = None
        if 'type="hidden" name="pass"' in result:
            passval = re.compile('name="pass" value="(.*?)"').findall(result)[0]
            query = '%s/cdn-cgi/l/chk_jschl?s=%s&pass=%s&jschl_vc=%s&jschl_answer=%s' % (u, urllib_parse.quote_plus(sval), urllib_parse.quote_plus(passval), jschl, answer)
            xbmc.sleep(4 * 1000)  # sleep so that the call work

        opener.addheaders = [('User-Agent', agent), ('Referer', url), ('Accept', 'text/html, application/xhtml+xml, application/xml, */*'), ('Accept-Encoding', 'gzip, deflate')]
