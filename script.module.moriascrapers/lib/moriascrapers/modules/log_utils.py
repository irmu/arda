...
DEBUGPREFIX = '[ MoriaScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ MoriaScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'moria.log')
debug_enabled = control.addon('plugin.video.moria').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('MoriaScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
