...
addonObject = addon('script.module.moriascrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from moriascrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.moria.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.moria.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.moria.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.moria.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.moria.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.moria.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def cdnImport(uri, name):
    import imp
    from moriascrapers.modules import client
