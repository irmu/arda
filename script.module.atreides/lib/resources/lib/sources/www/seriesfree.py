...
class source:
    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            result = re.compile(
                'itemprop="url"\s+href="([^"]+).*?itemprop="name"\s+class="serie-title">([^<]+)', re.DOTALL).findall(result)
            for i in result:
                if cleantitle.get(cleantitle.normalize(i[1])) in t and year in i[1]:
                    url = i[0]
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            url = [i.attrs['href'] for i in items if bool(re.compile(
                '<span\s*>%s<.*?itemprop="episodeNumber">%s<\/span>' % (season, episode)).search(i.content))][0]
        ...
        try:
            ...
            for link in links[:5]:
                ...
                try:
                    ...
                    for x in urls:
                        sources.append(
                            {'source': host, 'quality': x['quality'],
                             'language': 'en', 'url': x['url'],
