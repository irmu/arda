...
class seasons:
    ...
    def tvdb_list(self, tvshowtitle, year, imdb, tvdb, lang, limit=''):
        ...
        try:
            ...
            artwork = [i for i in artwork if 'seasonswide' not in re.findall('<BannerPath>(.+?)</BannerPath>', i)[0]]
            ...
            episodes = [
                i for i in episodes
                if '<SeasonNumber>0</SeasonNumber>' not in i or control.setting('show_season0') != 'false']
            episodes = [i for i in episodes if '<EpisodeNumber>0</EpisodeNumber>' not in i]
        ...
        for item in episodes:
            try:
                ...
                self.list.append({'title': title, 'label': label, 'season': season, 'episode': episode,
                                  'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': status,
                ...
                cm.append(
                    (playRandom,
                     ...
                     (sysaddon, urllib.quote_plus(systitle),
                      ...
                      urllib.quote_plus(season))))
                cm.append((queueMenu, 'RunPlugin(%s?action=queueItem)' % sysaddon))
                cm.append(
                    (watchedMenu, 'RunPlugin(%s?action=tvPlaycount&name=%s&imdb=%s&tvdb=%s&season=%s&query=7)' %
                     (sysaddon, systitle, imdb, tvdb, season)))
                cm.append(
                    (unwatchedMenu, 'RunPlugin(%s?action=tvPlaycount&name=%s&imdb=%s&tvdb=%s&season=%s&query=6)' %
                     (sysaddon, systitle, imdb, tvdb, season)))
...
class episodes:
        ...
        try:
            if idx is True:
                if season is None and episode is None:
                    ...
                    self.list = cache.get(seasons().tvdb_list, 1, tvshowtitle, year, imdb, tvdb, self.lang, '-1')
                    num = [x for x, y in enumerate(self.list) if y['season'] == str(season)
                           and y['episode'] == str(episode)][-1]
    ...
    def calendars(self, idx=True):
        ...
        try:
            months = [(m[0], 'January'), (m[1], 'February'), (m[2], 'March'), (m[3], 'April'), (m[4], 'May'), (m[5], 'June'),
        ...
        try:
            days = [(d[0], 'Monday'), (d[1], 'Tuesday'), (d[2], 'Wednesday'),
                    (d[3], 'Thursday'), (d[4], 'Friday'), (d[5], 'Saturday'), (d[6], 'Sunday')]
        except Exception:
            days = []
    ...
    def trakt_progress_list(self, url, user, lang):
        ...
        for item in result:
            try:
                ...
                items.append({'imdb': imdb, 'tvdb': tvdb, 'tvshowtitle': tvshowtitle, 'year': year,
                              'snum': season, 'enum': episode, '_last_watched': last_watched})
            except Exception:
                pass
        ...
        def items_list(i):
            try:
                item = [x for x in self.blist if x['tvdb'] == i['tvdb']
                        and x['snum'] == i['snum'] and x['enum'] == i['enum']][0]
            ...
            try:
                ...
                num = [x for x, y in enumerate(item) if re.compile('<SeasonNumber>(.+?)</SeasonNumber>').findall(y)[0] == str(
                    i['snum']) and re.compile('<EpisodeNumber>(.+?)</EpisodeNumber>').findall(y)[0] == str(i['enum'])][-1]
    ...
    def trakt_episodes_list(self, url, user, lang):
        ...
        def items_list(i):
            try:
                item = [x for x in self.blist if x['tvdb'] == i['tvdb']
                        and x['season'] == i['season'] and x['episode'] == i['episode']][0]
                if item['poster'] == '0':
                    raise Exception()
                self.list.append(item)
                return
