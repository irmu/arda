...
from resources.lib.modules import cleantitle, control
...
class libtvshows:
    ...
    def add(self, tvshowtitle, year, imdb, tvdb, range=False):
        ...
        try:
            items = [
                {'title': i['title'],
                 ...
                 'premiered': i['premiered']} for i in items]
        except Exception:
            items = []
        ...
        try:
            ...
            lib = [
                ...
                if
                str(i['imdbnumber']) in id
                or(i['title'].encode('utf-8') == items[0]['tvshowtitle'] and str(i['year']) == items[0]['year'])][0]
    ...
    def strmFile(self, i):
        try:
            title, year, imdb, tvdb, season, episode, tvshowtitle, premiered = i['title'], i['year'], i[
                'imdb'], i['tvdb'], i['season'], i['episode'], i['tvshowtitle'], i['premiered']
            ...
            lib_tools.create_folder(folder)
            lib_tools.write_file(os.path.join(folder, lib_tools.legal_filename('%s S%02dE%02d' %
                                                                               (transtitle, int(season), int(episode))) + '.strm'), content)
        except Exception:
            pass
        ...
        for item in items:
            ...
            try:
                ...
                ep = [
                    ...
                    if
                    str(x['imdbnumber']) in id
                    or(x['title'].encode('utf-8') == item['tvshowtitle'] and str(x['year']) == item['year'])][0]
                ep = control.jsonrpc(
                ...
                num = [x for x, y in enumerate(it) if str(y['season']) == str(
                    ep['season']) and str(y['episode']) == str(ep['episode'])][-1]
