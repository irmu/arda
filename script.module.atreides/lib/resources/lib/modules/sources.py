...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ...
        source_types = self.getSourceTypes()
        sourceDict = [(i[0], i[1], i[1].source) for i in sourceDict]
        sourceDict = [(i[0], i[1]) for i in sourceDict if any(x in i[2] for x in source_types)]
    ...
    def sourcesFilter(self):
        ...
        sortabitch = control.setting('torrent.sort.a.bitch')
        if sortabitch == '':
            sortabitch = 'false'
        ...
        try:
            if not ThreeD == 'true':
                self.sources = [i for i in self.sources if '3D' not in i['label']]
        except Exception:
            pass
    ...
    def getSourceTypes(self):
        sTypes = []
        www = control.setting('www.enabled')
        if www == '' or www == 'true':
            sTypes.append('www')
        openlist = control.setting('openlist.enabled')
        if openlist == '' or openlist == 'true':
            sTypes.append('openlist')
        if debrid.status(True):
            sTypes.append('magnet')
        return sTypes
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.atreides.container.items'

        self.metaProperty = 'plugin.video.atreides.container.meta'
