...
#TC 2/01/19 started
        ...
        if control.addonInfo('id') == 'plugin.video.thecrew':
            try:
                if progressDialog: progressDialog.update(100, six_ensure_str(control.lang(30726)), six_ensure_str(control.lang(30731)))
    ...
    def sourcesFilter(self):
        ...
        if quality <= 1:
            filter += [i for i in self.sources if i['quality'] == ['1440p','1440P'] and 'debrid' in i]
        if quality <= 1:
            filter += [i for i in self.sources if i['quality'] == ['1440p','1440P'] and 'debrid' not in i and 'memberonly' in i]
        if quality <= 1:
            filter += [i for i in self.sources if i['quality'] == ['1440p','1440P'] and 'debrid' not in i and 'memberonly' not in i]


        if quality <= 2:
            filter += [i for i in self.sources if i['quality'] in ['1080p', '1080P'] and 'debrid' in i]
        if quality <= 2:
            filter += [i for i in self.sources if i['quality'] == ['1080p', '1080P'] and 'debrid' not in i and 'memberonly' in i]
        if quality <= 2:
            filter += [i for i in self.sources if i['quality'] == ['1080p', '1080P'] and 'debrid' not in i and 'memberonly' not in i]
        ...
        if quality <= 3:
            filter += [i for i in self.sources if i['quality'] == ['720p', '720P'] and 'debrid' not in i and 'memberonly' in i]
        if quality <= 3:
            filter += [i for i in self.sources if i['quality'] == ['720p', '720P'] and 'debrid' not in i and 'memberonly' not in i]
