...
from resources.lib.modules import cache, dom_parser, log_utils, control, hunter
...
def tinyjs(url):
    ...
    h = unquote(t)
    hat = re.findall('(?s)var count .*?location.href =\s*"([^"]*)', h)[0]
    return hat

def tinyu(url):
    u = request(url)
    e = re.compile('decode.*?"([^"]*)",([^,]*),"([^"]*)",([^,]*),([^,]*),([^\)]*)').findall(u)[0]
