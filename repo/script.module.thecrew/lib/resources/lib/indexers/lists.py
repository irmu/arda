...
#OH - checked
    def getx(self, url, worker=False):
        try:
            ...
            x = regex.fetch(x)
            log_utils.log('[CM Debug @ 377 in lists.py] tekst =' + repr(x))   
            #r += urllib_parse.unquote_plus(x) <- # OH is this correct??
            r += urllib.parse.unquote_plus(x) # <- i think this should be it??
            log_utils.log('[CM Debug @ 380 in lists.py] tekst =' + str(r))   
            url = regex.resolve(r)
            log_utils.log('[CM Debug @ 377 in lists.py] tekst =' + repr(url))   
