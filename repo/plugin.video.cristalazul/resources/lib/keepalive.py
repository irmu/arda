...
class ConnectionManager:
    ...
    def add(self, host, connection, ready):
        self._lock.acquire()
        try:
            if host not in self._hostmap.keys():
                self._hostmap[host] = []
...
class HTTPHandler(KeepAliveHandler, urllib_request.HTTPHandler):
...
class HTTPSHandler(KeepAliveHandler, urllib_request.HTTPSHandler):
