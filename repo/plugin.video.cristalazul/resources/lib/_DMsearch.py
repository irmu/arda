...
addon = xbmcaddon.Addon('plugin.video.cristalazul_3.0')
profile = six.ensure_str(TRANSLATEPATH(addon.getAddonInfo('profile')))
...
def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, duration, total):
        ...
        ok = True
        mode = '12'
        contextMenu.append(('[COLOR white]!!Download Currently Playing!![/COLOR]', 'XBMC.RunPlugin(%s?url=%s&mode=21&name=%s)' % (sys.argv[0], urllib_parse.quote_plus(url), urllib_parse.quote.quote_plus(name))))             
...
def notification(header="", message="", sleep=3000):
    ...
    """
    xbmc.executebuiltin("Notification(%s,%s,%i)" % (header, message, sleep))     
