...
class get(object):
    ...
    def worker(self, url):
        try:
            ...
            u = urllib_parse.urlparse(url).netloc
...
class captcha:
    ...
    def solvemedia(self, data):
        try:
            ...
            getUrl('http://api.solvemedia.com/papi/verify.noscript', post=urllib_parse.urlencode(post)).result
...
class premiumize:
    ...
    def resolve(self, url):
        try:
            ...
            url = 'http://api.premiumize.me/pm-api/v1.php?method=directdownloadlink&params[login]=%s&params[pass]=%s&params[link]=%s' % (self.user, self.password, urllib_parse.quote_plus(url))
...
class realdebrid:
    ...
    def resolve(self, url):
        try:
            ...
            login_data = urllib_parse.urlencode({'user': self.user, 'pass': self.password})
    ...
    def resolve(self, url):
        try:
            if 'picasaweb' in url.lower():
                ...
                pid = urllib_parse.urlparse(url).fragment
                oid = re.compile('/(\d*)/').findall(urllib_parse.urlparse(url).path)[0]
                key = urllib_parse.parse_qs(urlparse.urlparse(url).query)['authkey'][0]
...
class mailru:
    ...
    def resolve(self, url):
        try:
            ...
            h = "|Cookie=%s" % urllib_parse.quote(cookie)
...
class mybeststream:
    ...
    def resolve(self, url):
        try:
            referer = urllib_parse.parse_qs(urlparse.urlparse(url).query)['referer'][0]
...
class veehd:
    ...
    def resolve(self, url):
        try:
            ...
            url = urllib_parse.urljoin('http://veehd.com', url)

            result = getUrl(url, close=False).result

            i = common.parseDOM(result, "iframe", ret="src")
            if len(i) > 0:
                i = urllib_parse.urljoin('http://veehd.com', i[0])
            ...
            url = urllib_parse.unquote(url[0])
            return url
        except:
            return
...
class videomega:
    ...
    def resolve(self, url):
        try:
            ...
            url = urllib_parse.parse_qsl(url)[0][1]
