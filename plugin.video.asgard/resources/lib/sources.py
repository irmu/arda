...
class Sources(object):
    ...
    def get_sources(
        ...
        if ADDON.getSetting("use_link_dialog") == "true" and not skip_selector:
            # use link selector
            if content == "movie":
                scraper = universaldebrid.scrape_movie_with_dialog
                link, rest = scraper(
                    title,
                    year,
                    imdb,
                ...
                scraper = universaldebrid.scrape_episode_with_dialog
                link, rest = scraper(
                    tvshowtitle,
                    year,
                    premiered,
            ...
            if ADDON.getSetting("link_fallthrough") == "true":
                ...
                while not played:
                    try:
                        ...
                        pass
                    try:
                        resolved_link = universaldebrid.resolve(
                ...
                resolved_link = universaldebrid.resolve(link["scraper"], link["url"])
            ...
            if content == "movie":
                title = title
                scraper = universaldebrid.scrape_movie
                ...
                scraper = universaldebrid.scrape_episode
                links_scraper = scraper(
                    tvshowtitle,
                    year,
                    premiered,
        ...
        non_direct_links = []
        non_direct_sd_links = []
        num_scrapers = len(universaldebrid.relevant_scrapers())
        index = 0
        try:
                ...
                if scraper_links is not None:
                    ...
                    for scraper_link in scraper_links:
                        ...
                        if scraper_link["direct"]:
                            resolved_link = universaldebrid.resolve(scraper_link["scraper"], scraper_link["url"])
    ...
    def get_music_sources(
        ...
        allow_debrid = ADDON.getSetting("allow_debrid") == "true"
        if ADDON.getSetting("use_link_dialog") == "true" and not skip_selector:
            link, rest = universaldebrid.scrape_song_with_dialog(
                title,
                artist,
                timeout=timeout,
                exclude=exclude,
        ...
        links_scraper = universaldebrid.scrape_song(
